...
The dataset contains 2060 datapoints, split into 515 datapoints per year, one per each county. Given the even spread of 4 datapoints per county, all datapoints were treated equally. First the data was cropped to include values only for the time period from start of April to end of July. This data was then combined into a single csv file with the following steps: 
+ Maximum temperature measurements and precipitation measurements were combined with the label vector.
+ Soil moisture and VOD were processed as separate csv files due to the large number of measurements.
...
Soil moisture was chosen to be represented by three values: the arithmetic mean, the 5th percentile, and the 95th percentile. The mean represents overall differences in average soil moisture. The 5th and 95th percentiles take into account exceptionally dry and humid conditions, respectively, while being slightly less susceptible to extreme outliers.

The selected features and their cross-correlations are shown in @correlation2.
...
Data was split at 60 % for training, 20 % for validation, and 20 % for testing. 
