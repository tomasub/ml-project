...
#let lab-report(
  ...
  compact-mode: false,
  heading-color: black,
  heading-font: "Times New Roman", // recommended alternatives: "Fira Sans", "Lato", "Source Sans Pro"
      ...
      footer:
        context [
            ...
            #grid(
              ...
              number-type: "lining",
              context {if in-outline.get() {
                none      // no page number for the TOC
                } else {
                counter(page).display("1")      // arabic page numbers for the rest of the document
                }
              }
 ...
 set table(
  ...
  set figure(
    supplement: if language == "fi" {none} else {auto}
  )

  show figure.caption: it => if language == "fi" {if it.kind == table {
  [
    Taulukko
    ...
    [Kuva #it.counter.display(it.numbering),
    #(it.body)]
  }} else {[#(it)]}
 ...
 //   in-outline.update(false)
  ...
  // top-level TOC entries in bold with filling
